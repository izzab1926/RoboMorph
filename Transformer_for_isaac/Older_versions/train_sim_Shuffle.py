...
if __name__ == '__main__':
    ...
    parser.add_argument('--manuel_pc',default=False,   
    ...
    if cfg.custom_dataset == False:
        ...
        if cfg.log_wandb:
            wandb.init(
                project="sysid-meta-Isaac-gym-Franka",
        ...
        try:
            relative_folder = f"isaacgym/python/examples/Franka/out_tensors/{cfg.train_folder}"
        ...
        # ------------------------------------------------------------------------------
        # In this case is possible to merge multiple files.
        # Example: I have chirp and multi-sinusoidal, i merge them and in a 16-size
        # batch i obtain multiple tests.

        pick_number = 3

        for i in range(0,number_of_pt_files,pick_number):

            print(f"--------------Merging {pick_number} file ------------")
