...
from original.transformer_onestep import GPTConfig, GPT, warmup_cosine_lr
import tqdm
import argparse
import wandb
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Meta system identification with transformers (EDITED)')
    ...
    parser.add_argument('--in-file', type=str, default="ckpt_one_STEP_MANUEL", metavar='S',
    ...
    # Dataset
    parser.add_argument('--nx', type=int, default=9, metavar='N',
                        help='model order (default: 5)')
    parser.add_argument('--nu', type=int, default=7, metavar='N',
                        help='model order (default: 9)')
    parser.add_argument('--ny', type=int, default=7, metavar='N',
                        help='model order (default: 3)')
    parser.add_argument('--seq-len', type=int, default=700, metavar='N',
    ...
    # Model
    parser.add_argument('--n-layer', type=int, default=12, metavar='N', # original 12
                        help='number of iterations (default: 1M)')
    parser.add_argument('--n-head', type=int, default=4, metavar='N',
                        help='number of iterations (default: 1M)')
    parser.add_argument('--n-embd', type=int, default=400, metavar='N',
                        help='number of iterations (default: 1M)')
...
# ---------------------------------------------------------------------------------------------------
    ...
    if cfg.custom_dataset ==False:
        ...
        #     tensors_path = os.path.join(parent_folder, "isaacgym/python/examples/Franka/out_tensors/training") 
        ...
        #         print(single_pt_file_path)
        #         loaded = torch.load(single_pt_file_path)
        ...
        def loading():
            ...
            control_action = control_action[:,:600,:]
        ...
        cfg.in_file = f"{cfg.out_file}_nu{cfg.nu}_ny{cfg.ny}_seq_len{cfg.seq_len}"
        ...
        # cfg.in_file = f"{cfg.out_file}_nu{cfg.nu}_nu{cfg.ny}_seq_len{cfg.seq_len}"
        # cfg.init_from = "resume"
...
# ---------------------------------------------------------------------------------------------------
    ...
    torch.save(checkpoint, model_dir / f"{cfg.out_file}_nu{cfg.nu}_ny{cfg.ny}_seq_len{cfg.seq_len}.pt")
