...
parser.add_argument('--test-folder', type=str, default="testB", metavar='test',
                    help='test-folder')
parser.add_argument('--figure-folder', type=str, default="figure", metavar='figure',
                    help='Figure output')
...
try:
    relative_folder = f"isaacgym/python/examples/Franka/out_tensors/{test_cfg.test_folder}"
    ...
    for test_idx in range(len(list_of_available_tensors)): 
        ...
        # -------------------------------Metrics PLOT -----------------------------
            ...
            if save_figure == True:
                ...
                plt.close()
            # torch.save((y_ground_truth,y_predicted),test_path / "ground_truth_and_prediction.pt") 
            ...
            for idx in range(len(test_idxs)):
                ...
                fig, axs = plt.subplots(ny, 1,figsize=(20, 20))
                fig.suptitle('Prediction of y - robot idx '+ str(test_idxs[idx]) + 
                            ' ['+ str(label_idxs[idx])+'] ' + str(round(values[idx].item(),4)) ,size = 24,y = .99)
                ...
                for j in range(1):
                    for i in range(ny):
                        axs[i].plot(t, y_ground_truth[idx_plot, :, k] ,'k', label=labels_coordinates[k],linewidth=2)
                        axs[i].plot(t, y_predicted[idx_plot, :, k], 'b', label=labels_pred[k],linewidth=2)
                        axs[i].plot(t, sim_error[idx_plot, :, k], 'r', label=labels_error[k])
            ...
            for j in range(1):
                for i in range(ny):
                    ...
                    axs[i].axvline(x = t_prediction[-1], color = 'k', linestyle='--')
                    axs[i].axvline(x = t_context[-1], color = 'k', linestyle='--')
                    axs[i].fill_between( t_context, min_value, max_value, facecolor='lime', alpha=0.2)
                    # Plot prediction & error
                    axs[i].plot(t_prediction, y_predicted[random_idx, :, k], 'b', label=labels_pred[k],linewidth=2)
                    axs[i].plot(t_prediction, sim_error[random_idx, :, k], 'r', label=labels_error[k])
                    # Plot entire Signal
                    axs[i].plot(t_total, position[random_idx, :, k], 'k', label=labels_coordinates[k])
                    axs[i].legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
                    axs[i].grid(True)
                    k=k+1
...
for i in range(len(list_of_available_models)):
    ...
    # axes[0].plot(torch.ones(tests_count.shape),'b--',linewidth=2)
