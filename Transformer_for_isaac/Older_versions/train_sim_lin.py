...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval-interval', type=int, default=20, metavar='N',
                        help='batch size (default:32)')
    parser.add_argument('--eval-iters', type=int, default=20, metavar='N',
                        help='batch size (default:32)')
    ...
    # loaded = torch.load('tensor_u_and_position_BIG.pt')
    # control_action = loaded['control_action']
    # position = loaded['position']

    # # resize according to standard
    # seq_length, batch, dimension = control_action.shape
    # seq_length1, batch1, dimension1 = position.shape
    # control_action = control_action.view(batch , seq_length , dimension)
    # position = position.view(batch1 , seq_length1 , dimension1)
    ...
    # train_dl = torch.utils.data.DataLoader(train_ds, batch_size=cfg.batch_size, num_workers=cfg.threads)
    # val_dl = DataLoader(val_ds, batch_size=cfg.eval_batch_size, num_workers=cfg.threads)
