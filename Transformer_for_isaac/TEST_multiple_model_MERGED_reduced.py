...
for model_idx in range(len(list_of_available_models)):
    ...
    for test_idx in range(len(list_of_available_tensors)): 
        ...
        def loading():
            ...
            position_extracted = loaded['position'][skip:,:test_cfg.num_test,:ny]
        ...
        RMSE_reshaped = metrics.rmse(y_ground_truth_reshaped, y_predicted_reshaped, time_axis=0).reshape(ny,1)
        NRMSE_reshaped = metrics.nrmse(y_ground_truth_reshaped, y_predicted_reshaped, time_axis=0).reshape(ny,1)
        fit_index_reshaped = metrics.fit_index(y_ground_truth_reshaped, y_predicted_reshaped, time_axis=0).reshape(ny,1)
        r_squared_reshaped = metrics.r_squared(y_ground_truth_reshaped, y_predicted_reshaped, time_axis=0).reshape(ny,1)
        ...
        for j in range(4):
            ...
            axes[j].boxplot(metrics_values_EE[j],1,'',patch_artist = True, 
                            ...
                            labels=[f"{quantities[i]}" for i in range(7)])
        ...
        for j in range(4):
            ...
            axes[j].boxplot(metrics_values_joint_pos[j],1,'',patch_artist = True, 
                            ...
                            medianprops = dict(color = "green", linewidth = 1.5), 
                            whiskerprops = dict(color = "red", linewidth = 2), 
                            labels=[f"{quantities[i+7]}" for i in range(7)])
