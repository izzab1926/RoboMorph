...
args = gymutil.parse_arguments(description="Data Generation for multiple kuka systems",
        custom_parameters=[
        {"name": "--num-envs", "type": int, "default": 16, "help": "Number of environments to create"},
        #General
        ...
        {"name": "--joints", "type": int, "default": 7, "help": "Actual number of dofs in which we are interested (arms)"},
...
for iter_run in range(num_of_runs):
    ...
    generated_seed = np.random.randint(0,999)
    ...
    # Load kuka asset
    asset_root = "./"
    # Specify the folder of the Kuka urdf file
    kuka_asset_file = "kuka_allegro_description/kuka.urdf"

    asset_options = gymapi.AssetOptions()
    asset_options.fix_base_link = True
    asset_options.flip_visual_attachments = False # CHANGED
    ...
    kuka_asset = gym.load_asset(
        sim, asset_root, kuka_asset_file, asset_options)
    # get joint limits and ranges for kuka
    kuka_dof_props = gym.get_asset_dof_properties(kuka_asset)

    # ------------ From Datasheet --------------
    kuka_dof_props['effort'][:2].fill(176)
    kuka_dof_props['effort'][2:4].fill(110)
    kuka_dof_props['effort'][5:7].fill(40)
    kuka_effort_limits = kuka_dof_props['effort'] 
    # ------------------------------------------
    ...
    default_dof_state = np.zeros(kuka_num_dofs, gymapi.DofState.dtype)
    default_dof_state["pos"][:4] = kuka_upper_limits[:4] * 0.5
    default_dof_state["pos"][5:7] = kuka_lower_limits[5:7] * 0.4

    # kuka_dof_props["stiffness"][:7].fill(0.0)
    # kuka_dof_props["damping"][:7].fill(0.0)
    ...
    # Added! 
    body_links =gym.get_asset_rigid_body_count(kuka_asset)
    ...
    for i in range(num_envs):
        ...
        envs.append(env)
        # Add kuka
        kuka_handle = gym.create_actor(env, kuka_asset, pose, "kuka", i, 0) # from 1 to 0
        ...
        if  not random_initial_positions:
            ...
            default_dof_state["pos"][0] =  0.1
            default_dof_state["pos"][1] =  math.pi/16
            default_dof_state["pos"][2] =  kuka_upper_limits[2] * 0.9 
            default_dof_state["pos"][3] =  kuka_upper_limits[3] * 0.7
            default_dof_state["pos"][4] =  0
            default_dof_state["pos"][5] =  kuka_lower_limits[5] * .7 
            default_dof_state["pos"][6] = kuka_lower_limits[6] * 0.5

        else:
            magnitude = torch.rand(1).uniform_(0.2,0.9).numpy()

            default_dof_state["pos"][0] =  0.1 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * torch.rand(1).uniform_(kuka_lower_limits[0],kuka_upper_limits[0]).numpy() 
            default_dof_state["pos"][1] =  math.pi/16 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()
            default_dof_state["pos"][2] =  kuka_upper_limits[2] * 0.9 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()
            default_dof_state["pos"][3] =  kuka_upper_limits[3] * 0.7 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()
            default_dof_state["pos"][4] =  0 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()
            default_dof_state["pos"][5] =  kuka_lower_limits[5] * .7 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()
            default_dof_state["pos"][6] = kuka_lower_limits[6] * 0.5 + np.sign(torch.rand(1).uniform_(-1,1).numpy()) * 0.25 * torch.rand(1).numpy()


        gym.set_actor_dof_states(env, kuka_handle, default_dof_state , gymapi.STATE_ALL)
        
        kuka_dof_props["driveMode"][:7].fill(gymapi.DOF_MODE_EFFORT)

        if  not random_stiffness_dofs:
            #set DOF control properties (except grippers)
            kuka_dof_props["stiffness"][:7].fill(0) 
            # kuka_dof_props["damping"][:7].fill(0.0) # default values inside URDF! 
        else:
            lower_bound_damping,higher_bound_damping,lower_random_stiff,higher_bound_stiff = 5, 10 ,0.01, 0.05 
            ...
            # set DOF control properties (except grippers)
            kuka_dof_props["driveMode"][:7].fill(gymapi.DOF_MODE_EFFORT)
            kuka_dof_props["stiffness"][:7]=random_stiff
            kuka_dof_props["damping"][:7]=random_damping
        ...
        kuka_dof_props["driveMode"][7:].fill(gymapi.DOF_MODE_POS)
        # kuka_dof_props["stiffness"][7:].fill(800.0)
        # kuka_dof_props["damping"][7:].fill(40.0)
        ...
        if  not random_masses:
            ...
            rigid_body_properties = gym.get_actor_rigid_body_properties(env, kuka_handle)
            ...
            for l in range(0,len(rigid_body_properties)):
                ...
                update = gym.set_actor_rigid_body_properties(env, kuka_handle, rigid_body_properties,0)
        ...
        hand_handle = gym.find_actor_rigid_body_handle(env, kuka_handle, "iiwa7_link_7")
        hand_pose = gym.get_rigid_transform(env, hand_handle)
        init_pos_list.append([hand_pose.p.x, hand_pose.p.y, hand_pose.p.z])
        init_orn_list.append([hand_pose.r.x, hand_pose.r.y, hand_pose.r.z, hand_pose.r.w])

        # Get global index of hand in rigid body state tensor
        hand_idx = gym.find_actor_rigid_body_index(env, kuka_handle, "iiwa7_link_7", gymapi.DOMAIN_SIM)
    ...
    # body_links = gym.get_asset_rigid_body_count(kuka_asset)
    # masses = torch.empty(num_envs,body_links, dtype=torch.float, device=args.graphics_device_id)
    
    # dof_count = gym.get_asset_dof_count(kuka_asset)

    # for i in range(num_envs):
        
    #     for j in range(len(rigid_body_properties)):
    #             masses[i,j] = rigid_body_properties[j].mass
    ...
    # For kuka, tensor shape is (num_envs, 10, 6, 9)
    _jacobian = gym.acquire_jacobian_tensor(sim, "kuka")
    jacobian = gymtorch.wrap_tensor(_jacobian)

    # Jacobian entries for end effector
    hand_index = gym.get_asset_rigid_body_dict(kuka_asset)["iiwa7_link_7"]
    j_eef = jacobian[:, hand_index - 1, :]

    # Prepare mass matrix tensor
    # For kuka, tensor shape is (num_envs, 9, 9)
    _massmatrix = gym.acquire_mass_matrix_tensor(sim, "kuka")
    ...
    if control_imposed and not control_imposed_file:
        if type_of_input == 'multi_sinusoidal':
            # Multi-sinusoidal 
            my_control_action = control_action_function(num_envs,max_iteration+1,joints,frequency,"kuka") # +1 for bugs
    ...
    while not condition_window  and itr <= max_iteration-1: # while not gym.query_viewer_has_closed(viewer):  #ORIGINAL
        ...
        if  osc_task == True:
            # radius = 0.05
            if itr ==1:
                radius = torch.rand((1,num_envs)).uniform_(0.01,0.08).to(device=args.graphics_device_id)
            ...
            # pos_des[:, 0] = init_pos[int(num_envs/2):, 0] 
            # pos_des[:, 0] = init_pos[int(num_envs/2):, 1] + math.sin(itr / 50) * radius #EDITED
            # pos_des[:, 0] = init_pos[int(num_envs/2):, 2] + math.cos(itr / 50) * radius #EDITED
        ...
        if compensate:              #if not disable_gravity_flag:   
            ...
            dof_count = gym.get_asset_dof_count(kuka_asset)
            g = torch.zeros(num_envs, dof_count, 6, 1, dtype=torch.float, device=args.graphics_device_id)
            g[:, :, 2, :] = 9.81
            ...
            # g = torch.zeros(num_envs, dof_count, 6, 1, dtype=torch.float, device=args.graphics_device_id)
            # g[:, :, 2, :] = 9.81
            # g_force = masses.unsqueeze(-1).unsqueeze(-1) *g
            # j_link = jacobian[:, :dof_count, :, :dof_count]
    ...
    # --- COLLISION DETECTION 
        ...
        body_contact = torch.nonzero(abs(contact_forces)> 30)
        ...
        for j in range(body_contact.shape[0]):
            ...
            if  not env_idx_collision in black_list : 
                ...
                for k in range(body_links):
                    gym.set_rigid_body_color(env_handle,kuka_handle, k , mesh ,color)
        ...
        if control_imposed and control_imposed_file:
            ...
            control_action = u 
            control_action = control_action.view(1,num_envs,7)
            control_action = control_action[:,:,:joints]

        if osc_task:
            if itr==1:
                print("\n Torque Imposed by OSC")
            control_action = u  
        ...
        dof_pos = dof_pos.to("cpu").view(1,num_envs,7)
        ...
        ll = torch.tensor(kuka_lower_limits[:7]).repeat(num_envs,1) 
        ul = torch.tensor(kuka_upper_limits[:7]).repeat(num_envs,1) 
        saturation_ll = torch.nonzero(abs(dof_pos[:,:,:7]-ll) < 0.005)
        saturation_ul = torch.nonzero(abs(dof_pos[:,:,:7]-ul) < 0.005)
        ...
        if osc_task:
            torques_limit = torch.tensor(kuka_effort_limits[:7]).repeat(num_envs,1).to(device=args.graphics_device_id) 
            ...
            # WIP for peaks in kuka
            # if itr > 100:

            #     rms_torques = torch.sqrt(torch.mean(torch.square(buffer_control_action[:,:,:itr-1]),dim=2))

            #     abs_increment = abs (abs(control_action.squeeze(-1)[:,:7]) - abs(buffer_control_action[:,:,:itr-1])) 
            #     abs_previous = abs (control_action.squeeze(-1)[:,:7] )
            #     torque_peak = torch.nonzero( (abs_increment / abs_previous  ) > 5 ) # incremento 300 %
            #     abnormal_idxs = torque_peak.shape[0]
            #     if torque_peak.shape[0] != 0:
            #         for j in range(torque_peak.shape[0]):
            #             saturated_ul_idxs.append(int(torque_peak[j,0].to('cpu').numpy())) # append in another                
            #         print(saturated_ul_idxs)
    ...
    if save_tensors and not(error_all_collided) and not(control_imposed_file): 
        ...
        name_tensor = (str(generated_seed) +'_KUKA_envs_' + str(num_valid_envs) + '_steps_' + str(max_iteration)+ 
        ...
        if osc_task:
            ...
            name_tensor = (str(generated_seed) +'_KUKA_envs_' + str(num_valid_envs) + '_steps_' + str(max_iteration)+ 
                            'xy_task_'+ str(int(random_initial_positions))+ str(int(random_stiffness_dofs)) + 
